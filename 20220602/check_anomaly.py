...
# %%
def label_anomaly(tbl, target, yaml, thres=0.7):
    _tbl = tbl
    _tbl["MBURST0"] = "-1" 

    b_start, b_end = detection(_tbl["TIME"], _tbl["MRATE0"], padding=30, factor = 3.0)
    ...
    _tbl.loc[_tbl["MRATE0"]<thres*_tbl["MRATE0"].std(),"MBURST0"] = "-1"
...
for num in range(len(obj_list)):
    ...
    _tbl = tbl_hardness(label_anomaly(make_table0(target), target, b, thres=fac))
    ...
    burst_num = _tbl["MBURST0"].astype(int).max()+1

    for i in range(0, burst_num):
        x=_tbl.loc[_tbl["MBURST0"]==str(i), "TIME"]
        y=_tbl.loc[_tbl["MBURST0"]==str(i), "MRATE0"]
        error_y=dict(array= _tbl.loc[_tbl["MBURST0"]==str(i), "MERROR0"])
        trace_burst= go.Scattergl(
            x=x, y=y, error_y= error_y,
            mode = "markers",
            marker=dict(color=minimax(_tbl.loc[_tbl["MBURST0"]==str(i), "TIME"]).values,
...
sns.set("talk")
fig, axes = plt.subplots(len(obj_list)//4, 4, tight_layout=True, figsize=(20, 30))
# fig, axes = plt.subplots(len(obj_list)//4, 4, tight_layout=True, figsize=(30, 50), sharex="all")
fig.suptitle(f"Hardness v.s. Intensity (>{fac} sigma)")
fig.supxlabel("Hardness (MAXI hard & MAXI soft)")
fig.supylabel("X-ray Intensity (counts/sm^2/sec)")
...
for num in range(len(obj_list)):
    ...
    for i in range(0, burst_num):
        list_symbol =["o", "s", "^", "d"]
        x=_tbl.loc[_tbl["MBURST0"]==str(i), "MHARDNESS"]
        y=_tbl.loc[_tbl["MBURST0"]==str(i), "INTENSITY"]
        error_y=dict(array= _tbl.loc[_tbl["MBURST0"]==str(i), "INT_ERROR"])

        cm=plt.get_cmap('RdBu') 
        cm_interval=[ i / (len(x) - 1) for i in range(len(x)) ]
        cm=cm(cm_interval)

        axes[(num)//4, (num)%4].scatter(x, y, marker=list_symbol[i], label = f"{obj_list.loc[num,'target']}_M_HI_burst", c=cm)
        # axes[(num)//4, (num)%4].errorbar(x, y, yerr=error_y, marker=list_symbol[i], label = f"{obj_list.loc[num,'target']}_M_HI_burst", c=cm)
    axes[(num)//4, (num)%4].set_yscale("log")
    axes[(num)//4, (num)%4].set_xscale("log")    
    axes[(num)//4, (num)%4].set_title(obj_list.loc[num,"target"])    

plt.savefig(f"/home/kurihara/2_lc_similarity/BATSURVEY/output/maxi/Analysis/{today}/HI_{fac}_color.png")
plt.savefig(f"/home/kurihara/2_lc_similarity/BATSURVEY/Ana_lc/{today}/HI_{fac}_color.png")

    #     trace_burst= go.Scattergl(
    #         x=x, y=y, error_y= error_y,
    #         mode = "markers",
    #         marker=dict(color=minimax(_tbl.loc[_tbl["MBURST0"]==str(i), "TIME"]).values,
    #                 colorscale="RdBu", showscale=True, symbol = list_symbol[i]),
...
#     for i in range(0, burst_num):
#         list_symbol =["circle", "square", "cross", "star-diamond"]
#         x=_tbl.loc[_tbl["MBURST0"]==str(i), "MHARDNESS"]
#         y=_tbl.loc[_tbl["MBURST0"]==str(i), "INTENSITY"]
#         error_y=dict(array= _tbl.loc[_tbl["MBURST0"]==str(i), "INT_ERROR"])
#         trace_burst= go.Scattergl(
